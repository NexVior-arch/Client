...
public class HudEditorScreen extends Screen {
	...
	/** Tracks left-button state from the previous frame, to detect a
	 *  fresh press (rising edge) without depending on Screen's own
	 *  mouseClicked() callback, whose parameter types have proven to
	 *  shift across recent Minecraft versions. */
	private boolean wasLeftDownLastFrame = false;
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		for (HudModule module : HudRenderer.getModules().values()) {
			...
			ModuleStyleRenderer.drawBorder(context, x - 2, y - 2, w + 4, h + 4, boxColor);
	...
	/**
	 * Handles all mouse interaction (click-to-start-drag, drag, and
	 * release) via per-frame polling rather than overriding Screen's
	 * mouseClicked()/mouseDragged()/mouseReleased() callbacks. Those
	 * callbacks' parameter types have proven to shift across recent
	 * Minecraft versions (button-press data is now wrapped in a "Click"
	 * record type in this version rather than passed as a plain int,
	 * confirmed the hard way via a CI compile failure), so this instead
	 * depends only on the stable render(DrawContext, int, int, float)
	 * signature and GLFW's own stable button-state API (via
	 * InputPollHandler, already used by the CPS module).
	 */
	private void pollInput(int mouseX, int mouseY) {
		boolean leftDown = InputPollHandler.isLeftDown();
		boolean freshPress = leftDown && !wasLeftDownLastFrame;
		wasLeftDownLastFrame = leftDown;

		if (draggingModule == null && freshPress) {
		...
		if (draggingModule != null) {
			if (!leftDown) {
				// Button released since last frame: finish the drag.
				draggingModule = null;
				snapGuideX = null;
				snapGuideY = null;
				ConfigManager.save();
	...
	private void tryStartDrag(int mouseX, int mouseY) {
		for (HudModule module : HudRenderer.getModules().values()) {
			ModuleConfig moduleConfig = ConfigManager.getModule(module.getId());
			int x = resolveX(module, moduleConfig);
			int y = resolveY(module, moduleConfig);
	...
	private void drawColorSwatch(DrawContext context, ModuleConfig config, int x, int y) {
		...
		ModuleStyleRenderer.drawBorder(context, x, y, COLOR_SWATCH_SIZE, COLOR_SWATCH_SIZE, 0xFFFFFFFF);
	}
