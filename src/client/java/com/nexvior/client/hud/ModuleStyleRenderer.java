...
public final class ModuleStyleRenderer {
	...
	/**
	 * Draws a 1px rectangle outline using fill() calls. Replaces
	 * DrawContext#drawBorder(int,int,int,int,int), which was found NOT to
	 * exist in the actual compiled 1.21.11 API (confirmed via a real
	 * compile error during CI — javadoc research had suggested it existed,
	 * but the resolved Yarn build at build-time disagreed). fill() is the
	 * most fundamental DrawContext primitive and is extremely unlikely to
	 * ever be missing, making this the safest possible replacement.
	 */
